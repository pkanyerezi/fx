...
<li id="<?php echo $data['id']; ?>" class="notification-item <?php echo $data['is_read']; ?>" style="position: relative;">
    <a id="<?php echo $data['id']; ?>" use-ajax-notifs class="no-ajax" href="<?php echo $this->Html->url(array('plugin' => 'notifications', 'controller' => 'notifications', 'action' => 'read', $data['id'])); ?>">
        <div>
    ...
    <?php echo $this->Html->link('x', array('plugin' => 'notifications', 'controller' => 'notifications', 'action' => 'delete', $data['id']), array('class' => 'deletelink no-ajax','style'=>'display:inline;float:right;margin-top:-30px;')); ?>
</li>