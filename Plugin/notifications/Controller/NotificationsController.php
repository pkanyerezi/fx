...
class NotificationsController extends NotificationsAppController {
	...
	public function getjsonnewlist($userId = null,$limit = 10) {
        $this->layout = 'ajax';
        if ($userId && $userId == AuthComponent::user('id')) {
            $this->set('result', $this->Notification->getNewList($userId, $limit));
        } else {
            $this->set('result', null);
        }
    }
	
	public function getonenew($userId = null,$limit = 1) {
        $this->layout = 'ajax';
        if ($userId && $userId == AuthComponent::user('id')) {
            $this->set('result', $this->Notification->getOneNew($userId, $limit));
        } else {
            $this->set('result', null);
        }
    }
