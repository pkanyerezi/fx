...
class Notification extends NotificationsAppModel {
    ...
    function getList($userId, $limit = 20, $only_new = true) {
	...
	function getNewList($userId, $limit = 20, $only_new = true) {
        $conditions[] = array($this->alias . '.user_id' => $userId);
        if ($only_new) {
            $conditions[] = array($this->alias . '.is_read' => 0);
        }
	...
	function getOneNew($userId, $limit = 1, $only_new = true) {
        $conditions[] = array($this->alias . '.user_id' => $userId);
        if ($only_new) {
            $conditions[] = array($this->alias . '.is_read' => 0);
        }
        $options = array(
            'conditions' => $conditions,
            'limit' => $limit,
            'order' => array($this->alias . '.is_read' => 'desc', $this->alias . '.created' => 'desc'),
        );
