<style>
	...
	#SoldReceiptDateMonth,#SoldReceiptDateDay,#SoldReceiptDateYear{width: 80px;}
</style>
				...
				<td>
					<div class="input select required"><label for="SoldReceiptInstrument">Instrument</label>
						<select name="data[SoldReceipt][instrument]" id="SoldReceiptInstrument">
							<option value="Cash" <?php if($this->data['SoldReceipt']['instrument']=='Cash') echo 'selected';?>>Cash</option>
							<option value="Cheque" <?php if($this->data['SoldReceipt']['instrument']=='Cheque') echo 'selected';?>>Cheque</option>
							<option value="TT" <?php if($this->data['SoldReceipt']['instrument']=='TT') echo 'selected';?>>TT</option>
						</select>
					</div>
				</td>
			</tr>			
...
<script>
	function updateCurrency(){
		...
		if(selected_currency=='c8'){
			currency_id = $('#SoldReceiptOtherCurrencyId').val();
		}
		<?php if($use_system_board_rates):?>
			//$('.rate').val($('#board'+currency_id).attr('sell'));
		<?php endif;?>
		updateUGX();
	}
	function updateUGX(){
	...
	$(document).ready(function(){
		...
		$(document).on('change','.my_currencies,.amount',function(){
	...
	function check_require_customer_details()
	{
		...
		var amountUGX = $('.amount_ugx').val();
		var rateUSD = $('#boardc1').attr('sell');
	...
	function require_customer_details(show)
	{
		<?php if($use_system_board_rates):?>
		if(show){
			$('.large_cash_detail').show();
			$('.large_cash_detail_required').html('');
			
			if(amountUSD>=amountMX){
				var requiredV = '<input name="requiredInput" value="SoldReceiptCustomerName|SoldReceiptNationality|SoldReceiptAddress|SoldReceiptPassportNumber|SoldReceiptPhoneNumber" type="hidden">';
