...
<?php
	...
	if(	isset($_REQUEST['receipt_number']) && !empty($_REQUEST['receipt_number']) &&
		...
		$q=$dbh->query(sprintf("SELECT COUNT(*) as c FROM purchased_receipts where id = '%s' limit 1",$receipt_id));
		if(count($q)>0){
			foreach($q as $row){
				if($row['c']){
					$dbh->query(sprintf("UPDATE purchased_receipts set status='$status' where id = '%s' limit 1",$receipt_id));
		...
		@$dbh->query(sprintf("INSERT into purchased_receipts(id,fox_id,customer_name,amount,purchased_purpose_id,rate,amount_ugx,currency_id,date,nationality,address,passport_number,status,user_id,name,orig_amount,orig_rate,other_name,other_currency_id) 
			values('$receipt_id','%d','%s','$amount','%s','$rate','$amount_ugx','%s','$date','%s','%s','%s','$status','%s','%s','$orig_amount','$orig_rate','$other_currency_name','$other_currency_id')",
