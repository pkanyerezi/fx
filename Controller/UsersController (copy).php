...
class UsersController extends AppController {
	...
	public $uses=array('User','ActionLog','Notifications.Notification','Creditor','Debtor','Receivable','Withdrawal','Currency','OtherCurrency','Opening','Safe');
	...
	public function safe_withdrawal_from_safe($currency_id,$amount){
		...
		if($amount<1){
			$this->Session->setFlash(__('Amount should be greater than 0'));
		...
		if($currency_id!='ugx'){
			if (!$this->Currency->exists($currency_id)) {
				$this->Session->setFlash(__('Currency not found'));
				$this->redirect(array('action'=>'view',$this->Auth->User('id')));
			}
		...
		if(!count($safe)){
			...
			if(!count($safe)){
				$this->Session->setFlash(__('No safe found.'));
				$this->redirect(array('action'=>'view',$this->Auth->User('id')));
			}
		}
		...
		if($amount > $safe['Safe'][''.$my_field]){
			$this->Session->setFlash(__('Insufficient amount left in the Safe. '.($safe['Safe'][''.$my_field]).' '.$$currency_description.' is left.'));
			$this->redirect(array('action'=>'view',$this->Auth->User('id')));
		}
		...
		if($this->Opening->save($this->request->data)){
			...
			if($this->Safe->save($this->request->data)){
				$this->Session->setFlash(__('Successfully withdrew '.$amount.' '.$currency_description));
				...
				$this->Session->setFlash(__('Failed to withdraw amount. Please try again.'));
			}
		}else{
			$this->Session->setFlash(__('Failed to update your amount withdrawn. Please try again or contact vendor if it persists.'));
		}
	...
	public function other_safe_withdrawal_from_safe($currency_id,$amount){
		...
		if(count($safe_data)){
			foreach($other_currencies as $other_currency){
				...
				if($other_currency['OtherCurrency']['id']==$currency_id){
					if($amount > $_amount){
						$this->Session->setFlash(__('Insufficient amount of '.($currency_id).' in safe. '.($_amount).' left.'));
						$this->redirect(array('action'=>'view',$this->Auth->User('id')));
					}
					$_amount-=$amount;
				}
			...
			//If there has not been any amount saved for any other currency
			$this->Session->setFlash(__('No currency found in the safe.'));
			$this->redirect(array('action'=>'view',$this->Auth->User('id')));
		}
	...
	//Done by a cashier
	public function safe_return_to_safe($currency_id,$amount){
		$amount=(int)$amount;
		$date_today	=($_REQUEST['date_today']);
		...
		if($amount > $opening_amount){
			$this->Session->setFlash(__('Insufficient amount on your account. '.($opening_amount).' '.$currency_description.' is left.'));
			$this->redirect(array('action'=>'view',$this->Auth->User('id')));
		}
		$new_opening_amount=$opening_amount-$amount;
		...
		if($this->Opening->save($this->request->data)){
			...
			$this->request->data['Safe'][''.$my_field]=($safe['Safe'][''.$my_field]+$amount);
			if($this->Safe->save($this->request->data)){
				$this->Session->setFlash(__('Successfully returned '.$amount.' '.$currency_description.' to safe.'));
				...
				$this->request->data['Opening'][''.$my_field]=$opening_amount;
				$this->Opening->save($this->request->data);
				$this->Session->setFlash(__('Failed to return the cash. Please try again.'));
			}
		}else{
			$this->Session->setFlash(__('Failed to update your amount returned in your account. Please try again or contact vendor if it persists.'));
		}
	...
	//Done by a cashier
	public function other_safe_return_to_safe($currency_id,$amount){
		$amount=(int)$amount;
		$date_today	=($_REQUEST['date_today']);
		...
		if($this->Opening->save($this->request->data)){
			...
			$this->request->data['Safe']['other_currencies']=json_encode($safe_arr);
			if($this->Safe->save($this->request->data)){
				$this->Session->setFlash(__('Successfully deposited '.$amount.' '.$currency_description));
				...
				$this->request->data['Opening'][''.$my_field]=$opening_amount;
				$this->Opening->save($this->request->data);
				$this->Session->setFlash(__('Failed to deposit amount. Please try again.'));
			}
		}else{
			$this->Session->setFlash(__('Failed to update your amount deposited. Please try again or contact vendor if it persists.'));
		}
	...
	public function safe_deposit_into_safe($currency_id,$amount){
		...
		if(!count($opening)){
			$this->Session->setFlash(__('No opening found for the next working day. Please save your work first.'));
			$this->redirect(array('action'=>'view',$this->Auth->User('id')));
		}
	...
	public function safe_send_to($user_id,$currency_id,$amount){
		...
		if (!$this->User->exists($user_id)) {
			$this->Session->setFlash(__('User/Cashier not found'));
			$this->redirect(array('action'=>'view',$this->Auth->User('id')));
		}
		...
		if(!count($destin_user)){
			$this->Session->setFlash(__('Cashier Not found.'));
			$this->redirect(array('action'=>'view',$this->Auth->User('id')));
		}
		...
		if($this->Opening->save($this->request->data)){
			...
			$this->request->data['Opening'][''.$my_field]=($destin_opening['Opening'][''.$my_field]+$amount);
			if($this->Opening->save($this->request->data)){
	...
	public function safe_deposit($currency_id,$amount){
		...
		if($this->Safe->save($this->request->data)){
			$this->Session->setFlash(__('Successfully deposited '.$amount.' '.$currency_description.' to safe.'));
			...
			$this->Session->setFlash(__('Failed to deposit into safe. Please try again.'));
		}		
		$this->redirect(array('action'=>'view',$this->Auth->User('id')));
	}
	...
	public function safe_withdraw($currency_id,$amount){
		...
		if($safe['Safe'][''.$my_field] < $amount){
			$this->Session->setFlash(__('Insufficient amount in the safe. '.($safe['Safe'][''.$my_field]).' '.$currency_description.' is left.'));
			$this->redirect(array('action'=>'view',$this->Auth->User('id')));
		}
		...
		if($this->Safe->save($this->request->data)){
			$this->Session->setFlash(__('Successfully withdrew '.$amount.' '.$currency_description.' from the safe.'));
			...
			$this->Session->setFlash(__('Failed to withdraw from the safe. Please try again.'));
		}
		$this->redirect(array('action'=>'view',$this->Auth->User('id')));
	}
	...
	public function other_safe_withdraw($currency_id,$amount){
		...
		if(count($data)){
			foreach($other_currencies as $other_currency){
				...
				if($other_currency['OtherCurrency']['id']==$currency_id){
					if($amount > $_amount){
						$this->Session->setFlash(__('Insufficient amount of '.($currency_id).'. '.($_amount).' left.'));
						$this->redirect(array('action'=>'view',$this->Auth->User('id')));
					}
					$_amount-=$amount;
				}
		...
		if($this->Safe->save($this->request->data)){
			...
			$this->Session->setFlash(__('Failed to withdraw into safe. Please try again.'));
		}	
 ...
 */
	...
	public function transfer($id){
		...
		if($this->request->is('post')){
			...
			if(!$this->Receivable->save(
				...
				$this->Session->setFlash(__('Failed Receivable'));
			...
			if(!$this->Withdrawal->save(
				...
				$this->Session->setFlash(__('Failed Withdrawal'));
				$this->redirect(array('action' => 'view',$id));
			}
			
			$this->Session->setFlash(__('Transfered'));
			$this->redirect(array('action' => 'view',$id));
		}
	...
	public function view($id = null,$dateRage=null){
		...
		$otherCurrencies=$this->OtherCurrency->find('all',array(
			'recursive'=>-1
		));
